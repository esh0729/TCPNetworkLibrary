...
namespace APMSocket
{
	...
	public class UserToken
	{
		...
		private void BeginReceive()
		{
			...
			{
				SocketError error;
				m_socket!.BeginReceive(m_receiveBuffer.buffer!, m_receiveBuffer.offset, m_receiveBuffer.count, SocketFlags.None, out error, new AsyncCallback(ProcessReceive), null);
			...
			{
				Console.WriteLine(String.Format("BeginReceive() {0}", ex.Message));
				Close();
			}
		}
		...
		private void ProcessReceive(IAsyncResult result)
		{
			...
			{
				SocketError error;
				int nReceiveBytes = m_socket!.EndReceive(result, out error);

				// 수신한 데이터 크기가 0 보다 크고 소켓에러가 성공일 경우
			...
			{
				Console.WriteLine(String.Format("ProcessReceive() {0}", ex.Message));
				Close();
			}
		}
		...
		private void BeginSend()
		{
			...
			{
				SocketError error;
				m_socket!.BeginSend(m_sendSegments, SocketFlags.None, out error, new AsyncCallback(ProcessSend), null);
			...
			{
				Console.WriteLine(String.Format("BeginSend() {0}", e.Message));
				Close();
			}
		}
		...
		private void ProcessSend(IAsyncResult result)
		{
			...
			{
				SocketError error;
				int nSendBytes = m_socket!.EndSend(result, out error);
		...
		private void OnSend(int nSendBytes)
		{
			lock (m_syncObject)
			{
				...
				{
					// 정상적인 비동기 송신의 경우 세그먼트 리스트의 모든 데이터를 SendBuffer에 입력 후 callback 호출
