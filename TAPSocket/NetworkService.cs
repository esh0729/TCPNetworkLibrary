...
namespace TAPSocket
{
	...
	public class NetworkService
	{
		...
		public NetworkService(int nMaxConnections, int nBufferSize, int nPreAllocBufferCount)
		{
			...
			m_bufferManager = new BufferManager(nMaxConnections * nBufferSize * nPreAllocBufferCount, nBufferSize);

			// 사용자토클풀 최대 접속자 수를 가진 풀의 크기로 생성
		...
		public void Initialize()
		{
			...
			// 모든 사용자 버퍼를 관리하는 배열 생성
			m_bufferManager.Initialize();

			// 사용자 토큰 생성 및 송신 및 수신 사용자 버퍼 할당 후 토큰에 설정
			UserToken token;
			for (int i = 0; i < m_nMaxConnections; i++)
			{
				UserBuffer receiveBuffer = new UserBuffer();
				m_bufferManager.SetBuffer(receiveBuffer);

				token = new UserToken(this, m_nBufferSize, receiveBuffer);
